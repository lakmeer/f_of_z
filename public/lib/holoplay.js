...
function HoloPlay(scene, camera, renderer, focalPointVector, constantCenter, hiResRender){
    ...
    function makeFullScreenButton () {
      ...
      buttonDiv.innerHTML = '<input type="button" style="margin:20px; position:fixed; top:0px; right:0px; z-index: 10000; height:50px; width:150px;" id="fullscreenButton" value="Enter Full Screen Mode"/>';
    ...
    function setupFullScreen () {
      var FSbutton = makeFullScreenButton();
    ...
    if (!calibration_obj){
        ...
        console.error("No Looking Glass display connected; using default calibration data. Please ensure your Looking Glass is connected to your computer via USB and reload the page.")
    } else {
    ...
    var ws = new WebSocket('ws://localhost:11222/');
		var finished = ws.close.bind(ws);
    
		var timeout = setTimeout(function () { 
			console.error("Calibration not found in internal memory.");
      ...
      initialized = true;
		...
		ws.onerror = function(event) {
			console.error("Three.js driver not detected! Click OK to download. If you have already installed the driver, please make sure port 11222 is open.");
			console.error("http://look.glass/threejsdriver");
      applyCalibration(null);
      initialized = true;
			finished();
		};
    ...
    HoloPlay.prototype.setupFullScreen = setupFullScreen;
    
    HoloPlay.prototype.reapplyCalibration = fixShaderValues;
